...
`romatch` を使って各フレームの `points3d_frame*.npy` / `colors_frame*.npy` を生成できます。
カメラ外部・内部パラメータはCOLMAPのスパースモデル（例: `sparse/0`）を利用します。

```bash
bash scripts/run_roma_triangulation.sh \
    /path/to/images \
...
詳細なオプションは `scripts/roma_triangulate_to_npy.py --help` を参照してください。
